...
public class SingleTreeNode {
    ...
    SingleTreeNode(PlayerTeamGParams p, Random rnd, int num_actions, Types.ACTIONS[] actions) {
        this(p, null, -1, rnd, num_actions, actions, 0, null);
    }

    private SingleTreeNode(PlayerTeamGParams p, SingleTreeNode parent, int childIdx, Random rnd, int num_actions,
    ...
    void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        //1.Selection, 2.Expansion, 3.Rollout and 4.Back Propagation
        double avgTimeTaken;
        ...
        while (!stop) {
            //Game state is copied so that the original game state is not modified
            GameState state = rootState.copy();
            //Elapsed CPU Timer is used to calculate the time elapsed to keep a track of budget constraints.
    ...
    private SingleTreeNode uct(GameState state) {
        ...
        for (SingleTreeNode child : this.children) {
            ...
            double uctValue = childValue +
                    params.K * Math.sqrt(Math.log(this.nVisits + 1) / (child.nVisits + params.epsilon)); //epsilon is added to avoid divide by zero error
